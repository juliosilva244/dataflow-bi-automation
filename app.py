...
def render_filters(df: pd.DataFrame, schema: dict) -> pd.DataFrame:
    ...
    if data_col and data_col in df_filtrado.columns:
        df_filtrado[data_col] = pd.to_datetime(
            df_filtrado[data_col],
            errors="coerce",
            dayfirst=True,
        )
    ...
    with col1:
        if primary_col and primary_col in df_filtrado.columns:
            primary_options = sorted(
                df_filtrado[primary_col].dropna().astype(str).unique().tolist()
            )
            ...
            primary_selected = st.multiselect(
                ...
                key="selected_primary_dimension",
            )

    with col2:
        if secondary_col and secondary_col in df_filtrado.columns:
            secondary_options = sorted(
                df_filtrado[secondary_col].dropna().astype(str).unique().tolist()
            )
            ...
            secondary_selected = st.multiselect(
                ...
                key="selected_secondary_dimension",
            )
    ...
    if primary_selected and primary_col and primary_col in df_filtrado.columns:
        df_filtrado = df_filtrado[
            df_filtrado[primary_col].astype(str).isin(primary_selected)
        ]

    if secondary_selected and secondary_col and secondary_col in df_filtrado.columns:
        df_filtrado = df_filtrado[
            df_filtrado[secondary_col].astype(str).isin(secondary_selected)
        ]
    ...
    if date_range and len(date_range) == 2 and data_col and data_col in df_filtrado.columns:
        ...
        df_filtrado = df_filtrado[
            (df_filtrado[data_col] >= start_date)
            & (df_filtrado[data_col] < end_date)
        ]
...
def render_schema_warning(analytics_state: dict) -> None:
    ...
    st.warning(
        "Atenção: algumas colunas esperadas não foram detectadas automaticamente: "
        + ", ".join(missing)
        + ". O dashboard continuará funcionando com os dados disponíveis."
    )
...
def render_debug_panel(df_filtrado: pd.DataFrame, analytics_state: dict) -> None:
    ...
    st.write("DEBUG — primary_dimension_summary")
    st.dataframe(
        charts.get("primary_dimension_summary", pd.DataFrame()),
        use_container_width=True,
    )

    st.write("DEBUG — secondary_dimension_summary")
    st.dataframe(
        charts.get("secondary_dimension_summary", pd.DataFrame()),
        use_container_width=True,
    )

    st.write("DEBUG — revenue_by_month")
    st.dataframe(
        charts.get("revenue_by_month", pd.DataFrame()),
        use_container_width=True,
    )

    st.write("DEBUG — revenue_by_category")
    st.dataframe(
        charts.get("revenue_by_category", pd.DataFrame()),
        use_container_width=True,
    )
    ...
    render_html(
        ...
        <div class="section-header">
            <div>
                ...
                <div class="section-subtitle">
                    Visão consolidada para apresentação, análise e tomada de decisão.
                </div>
            </div>
        </div>
        """
...
def render_export_page(analytics_state: dict) -> None:
    ...
    with col_excel:
        ...
        st.download_button(
            ...
            data=excel_file,
            file_name="relatorio_dataflow.xlsx",
    ...
    with col_pdf:
        ...
        st.download_button(
            ...
            data=pdf_file,
            file_name="relatorio_dataflow.pdf",
